...
bool Subtraction::check(string value1, string value2)
...
string Subtraction::calculate(string value1, string value2, string oper) {
	...
	for (int i = 0; i < secondQ.size(); i++) {
		...
		if (nd == numeratorOfNonDigits2 && secondQ.size() > 1) {
			...
			//cout<<value2<<secondQ[i]<<endl;
			value2 = calculate(secondQ[i], value2, "");
			value1 = secondQ[i + 1];
	...
	else
	{
		...
		else
		{
			...
			if (opern == " - " || opern == " + ") {
				...
				results = simplify(value1, "", "") + opern + simplify(value2, "", "");
			}
			else {
				results = simplify(value1, "", "") + " - " + simplify(value2, "", "");
			}
...
string Subtraction::simplify(string results, string oper, string oper2)
